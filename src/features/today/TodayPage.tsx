...
export function TodayPage() {
  ...
  const upcomingDeliveries = useMemo(() => {
    return orders
      .filter((order) => order.orderStatus === "active" && upcomingDateKeys.includes(getDateKeyFromDateTime(order.deliveryDateTime)))
    ...
    return [
      {
        key: "today",
        title: "Hoje",
        orders: upcomingDeliveries.filter((order) => getDateKeyFromDateTime(order.deliveryDateTime) === todayKey),
      },
      {
        key: "tomorrow",
        title: "Amanhã",
        orders: upcomingDeliveries.filter((order) => getDateKeyFromDateTime(order.deliveryDateTime) === tomorrowKey),
      },
      {
        key: "week",
        title: "Semana",
        orders: upcomingDeliveries.filter((order) => weekKeys.includes(getDateKeyFromDateTime(order.deliveryDateTime))),
      },
  ...
  const productionByDate = useMemo(() => {
    return deliveryDateKeysWithOrders.map((dateKey) => {
      ...
      upcomingDeliveries
        .filter((order) => getDateKeyFromDateTime(order.deliveryDateTime) === dateKey)
        .forEach((order) => {
          order.items.forEach((item) => {
            totalsByProduct.set(item.productName, (totalsByProduct.get(item.productName) ?? 0) + item.quantity);
          });
        });
      ...
      return {
        ...
        items: Array.from(totalsByProduct.entries())
          ...
          .sort((firstItem, secondItem) => firstItem.productName.localeCompare(secondItem.productName)),
      };
    });
  ...
  return (
    <div className="page-stack">
      ...
      {loading && <p className="muted-text">Carregando informações de hoje...</p>}
      ...
      <section className="today-overview-grid">
        <Card>
          <div className="today-metric-card">
            ...
            <strong>{upcomingDeliveries.length}</strong>
            <small>Hoje até {formatDateKeyBR(upcomingDateKeys[4])}</small>
          </div>
        </Card>
        ...
        <Card>
          <div className="today-metric-card">
            ...
            <strong>{contactSuggestions.length}</strong>
            ...
            productionByDate.map((dateProduction) => (
              <Card key={dateProduction.dateKey}>
                <div className="today-date-group">
                  <header>
                    <strong>{dateProduction.dateKey === todayKey ? "Hoje" : formatDateKeyShort(dateProduction.dateKey)}</strong>
                    <Badge>{`${dateProduction.items.length} ${dateProduction.items.length === 1 ? "produto" : "produtos"}`}</Badge>
                  </header>
              ...
              <Card key={deliveryCard.key}>
                <div className="today-date-group">
                  <header>
                    ...
                    <Badge>{`${deliveryCard.orders.length} ${deliveryCard.orders.length === 1 ? "entrega" : "entregas"}`}</Badge>
                  </header>
                    ...
                    {deliveryCard.orders.map((order) => (
                      <article className="today-list-item" key={order.id}>
                        <div>
                          ...
                          {deliveryCard.key === "week" && <span>{formatDateKeyShort(getDateKeyFromDateTime(order.deliveryDateTime))}</span>}
                        </div>
                  ...
                  return (
                    <article className="today-list-item" key={order.id}>
                      <div>
                        ...
                        <span>{getPaymentStatusLabel(paymentStatus)} · Restante {formatCurrencyBR(remaining)}</span>
                      </div>
                  ...
                  return (
                    <article className="today-list-item" key={client.id}>
                      <div>
                        ...
                        <span>{lastInteractionDateKey ? `Última interação: ${formatDateKeyBR(lastInteractionDateKey)}` : "Sem interação registrada"}</span>
                      </div>
